...
def normalize(table, column):
    ...
    min = table[0][column]
    max = table[0][column]
    for i in column_values(table, column):
        if i < min:
            min = i
        if i > max:
            max = i
    i = 0
    for row in table:
        table[i][column] = (row[column] - min)/(max-min)
        i += 1
    ...
    for row in table:
        if len(cut_points) == 1:
            ...
            for i in range(len(cut_points)-1):
                if cut_points[i] <= row[column] < cut_points[i+1]:
                    row[column] = i+2
            if row[column] >= cut_points[len(cut_points)-1]:
